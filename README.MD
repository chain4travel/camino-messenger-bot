...
If you don't already have a **CM account**, for development purposes a CM Account can be created via [camino messenger contracts](https://github.com/chain4travel/camino-messenger-contracts) `hardhat` tool or via [suite](https://dev.suite.camino.network/create) as explained [here](https://docs.camino.network/partners/partner-config/#step-by-step-configuration-guide).
...
```
The command will output an **address of your new CM Account** in format 0x<\...\>. Store it and add it to your .yml configuration under `cm_account_addess`.
