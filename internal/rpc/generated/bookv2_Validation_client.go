// Code generated by 'scripts/generate_grpc_service_handlers.sh'. DO NOT EDIT.
// template: templates/v1/client.go.tpl

package generated

import (
	"buf.build/gen/go/chain4travel/camino-messenger-protocol/grpc/go/cmp/services/book/v2/bookv2grpc"
	"github.com/chain4travel/camino-messenger-bot/v12/internal/messaging/message"
	"github.com/chain4travel/camino-messenger-bot/v12/internal/rpc"

	"google.golang.org/grpc"
)

const (
	ValidationServiceV2                      = "cmp.services.book.v2.ValidationService"
	ValidationServiceV2Request  message.Type = message.Type(ValidationServiceV2 + ".Request")
	ValidationServiceV2Response message.Type = message.Type(ValidationServiceV2 + ".Response")
)

var _ rpc.Client = (*ValidationV2Client)(nil)

func NewValidationServiceV2Client(grpcCon *grpc.ClientConn) *ValidationV2Client {
	client := bookv2grpc.NewValidationServiceClient(grpcCon)
	return &ValidationV2Client{client: client}
}

type ValidationV2Client struct {
	client bookv2grpc.ValidationServiceClient
}
