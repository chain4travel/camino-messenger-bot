...
func (s *accommodationv5AccommodationProductInfoServer) AccommodationProductInfo(ctx context.Context, request *accommodationv5.AccommodationProductInfoRequest) (*accommodationv5.AccommodationProductInfoResponse, error) {
	...
	responseIntf, err := s.reqHandler.HandleMessageRequest(ctx, AccommodationProductInfoServiceV5Request, request)
	switch {
	...
	response, ok := responseIntf.(*accommodationv5.AccommodationProductInfoResponse)
	if !ok {
		return s.errorResponse(typesv4.ErrorCode_ERROR_CODE_INTERNAL, fmt.Sprintf("invalid response type: expected %s, got %T", AccommodationProductInfoServiceV5Response, responseIntf)), nil
	}

	return response, nil
}

func (s *accommodationv5AccommodationProductInfoServer) errorResponse(code typesv4.ErrorCode, errorMessage string) *accommodationv5.AccommodationProductInfoResponse {
