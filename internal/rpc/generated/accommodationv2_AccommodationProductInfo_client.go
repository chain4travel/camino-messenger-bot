// Code generated by 'scripts/generate_grpc_service_handlers.sh'. DO NOT EDIT.
// template: templates/v1/client.go.tpl

package generated

import (
	"buf.build/gen/go/chain4travel/camino-messenger-protocol/grpc/go/cmp/services/accommodation/v2/accommodationv2grpc"
	"github.com/chain4travel/camino-messenger-bot/v12/internal/messaging/message"
	"github.com/chain4travel/camino-messenger-bot/v12/internal/rpc"

	"google.golang.org/grpc"
)

const (
	AccommodationProductInfoServiceV2                      = "cmp.services.accommodation.v2.AccommodationProductInfoService"
	AccommodationProductInfoServiceV2Request  message.Type = message.Type(AccommodationProductInfoServiceV2 + ".Request")
	AccommodationProductInfoServiceV2Response message.Type = message.Type(AccommodationProductInfoServiceV2 + ".Response")
)

var _ rpc.Client = (*AccommodationProductInfoV2Client)(nil)

func NewAccommodationProductInfoServiceV2Client(grpcCon *grpc.ClientConn) *AccommodationProductInfoV2Client {
	client := accommodationv2grpc.NewAccommodationProductInfoServiceClient(grpcCon)
	return &AccommodationProductInfoV2Client{client: client}
}

type AccommodationProductInfoV2Client struct {
	client accommodationv2grpc.AccommodationProductInfoServiceClient
}
