...
func (s *TransportSearchV5Client) Call(ctx context.Context, requestIntf protoreflect.ProtoMessage, opts ...grpc.CallOption) (protoreflect.ProtoMessage, message.Type) {
	messageType := TransportSearchServiceV5Response

	request, ok := requestIntf.(*transportv5.TransportSearchRequest)
	if !ok {
		return s.errorResponse(typesv4.ErrorCode_ERROR_CODE_INTERNAL, fmt.Sprintf("invalid request type: expected %s, got %T", TransportSearchServiceV5Request, requestIntf)), messageType
	}
...
func (s *TransportSearchV5Client) InvalidProtoErrResponseAndType(errorMessage string) (protoreflect.ProtoMessage, message.Type) {
	return s.errorResponse(typesv4.ErrorCode_ERROR_CODE_INVALID_PROTO, errorMessage), TransportSearchServiceV5Response
}

func (s *TransportSearchV5Client) errorResponse(code typesv4.ErrorCode, errorMessage string) *transportv5.TransportSearchResponse {
