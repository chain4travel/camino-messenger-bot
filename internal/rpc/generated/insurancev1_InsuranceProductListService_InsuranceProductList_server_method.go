...
func (s *insurancev1InsuranceProductListServiceServer) InsuranceProductList(ctx context.Context, request *insurancev1.InsuranceProductListRequest) (*insurancev1.InsuranceProductListResponse, error) {
	...
	responseIntf, err := s.reqHandler.HandleMessageRequest(ctx, InsuranceProductListServiceV1Request, request)
	if err != nil {
		return s.errorResponse(err.Error()), nil
	}

	response, ok := responseIntf.(*insurancev1.InsuranceProductListResponse)
	if !ok {
		return s.errorResponse(fmt.Sprintf("invalid response type: expected %s, got %T", InsuranceProductListServiceV1Response, response)), nil
	}

	return response, nil
}

func (s *insurancev1InsuranceProductListServiceServer) errorResponse(errorMessage string) *insurancev1.InsuranceProductListResponse {
