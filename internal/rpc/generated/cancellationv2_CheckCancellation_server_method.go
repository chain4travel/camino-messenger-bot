...
func (s *cancellationv2CheckCancellationServer) CheckCancellation(ctx context.Context, request *cancellationv2.CheckCancellationRequest) (*cancellationv2.CheckCancellationResponse, error) {
	...
	if !ok {
		return s.errorResponse(typesv4.ErrorCode_ERROR_CODE_INTERNAL, fmt.Sprintf("invalid response type: expected %s, got %T", CheckCancellationServiceV2Response, responseIntf)), nil
	}

	return response, nil
}

func (s *cancellationv2CheckCancellationServer) errorResponse(code typesv4.ErrorCode, errorMessage string) *cancellationv2.CheckCancellationResponse {
