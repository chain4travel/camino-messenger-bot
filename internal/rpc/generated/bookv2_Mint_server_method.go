...
func (s *bookv2MintServer) Mint(ctx context.Context, request *bookv2.MintRequest) (*bookv2.MintResponse, error) {
	...
	responseIntf, err := s.reqHandler.HandleMessageRequest(ctx, MintV2Request, request)
	if err != nil {
	...
	if !ok {
		return s.errorResponse(fmt.Sprintf("invalid response type: expected %s, got %T", MintV2Response, response)), nil
	}

	return response, nil
}

func (s *bookv2MintServer) errorResponse(errorMessage string) *bookv2.MintResponse {
