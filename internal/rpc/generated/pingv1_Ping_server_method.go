...
func (s *pingv1PingServer) Ping(ctx context.Context, request *pingv1.PingRequest) (*pingv1.PingResponse, error) {
	...
	responseIntf, err := s.reqHandler.HandleMessageRequest(ctx, PingV1Request, request)
	if err != nil {
	...
	if !ok {
		return s.errorResponse(fmt.Sprintf("invalid response type: expected %s, got %T", PingV1Response, response)), nil
	}

	return response, nil
}

func (s *pingv1PingServer) errorResponse(errorMessage string) *pingv1.PingResponse {
