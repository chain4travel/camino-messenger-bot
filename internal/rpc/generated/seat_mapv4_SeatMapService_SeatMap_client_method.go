...
func (s SeatMapServiceV4Client) Call(ctx context.Context, requestIntf protoreflect.ProtoMessage, opts ...grpc.CallOption) (protoreflect.ProtoMessage, types.MessageType, error) {
	messageType := SeatMapServiceV4Response

	request, ok := requestIntf.(*seat_mapv4.SeatMapRequest)
	if !ok {
		return s.errorResponse(fmt.Sprintf("invalid request type: expected %s, got %T", SeatMapServiceV4Request, requestIntf)), messageType, nil
	}
...
func (s *SeatMapServiceV4Client) ErrorResponseAndType(errorMessage string) (protoreflect.ProtoMessage, types.MessageType) {
	return s.errorResponse(errorMessage), SeatMapServiceV4Response
}

func (s *SeatMapServiceV4Client) errorResponse(errorMessage string) protoreflect.ProtoMessage {
