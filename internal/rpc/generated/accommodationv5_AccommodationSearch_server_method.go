...
func (s *accommodationv5AccommodationSearchServer) AccommodationSearch(ctx context.Context, request *accommodationv5.AccommodationSearchRequest) (*accommodationv5.AccommodationSearchResponse, error) {
	...
	responseIntf, err := s.reqHandler.HandleMessageRequest(ctx, AccommodationSearchServiceV5Request, request)
	switch {
	...
	response, ok := responseIntf.(*accommodationv5.AccommodationSearchResponse)
	if !ok {
		return s.errorResponse(typesv4.ErrorCode_ERROR_CODE_INTERNAL, fmt.Sprintf("invalid response type: expected %s, got %T", AccommodationSearchServiceV5Response, responseIntf)), nil
	}

	return response, nil
}

func (s *accommodationv5AccommodationSearchServer) errorResponse(code typesv4.ErrorCode, errorMessage string) *accommodationv5.AccommodationSearchResponse {
