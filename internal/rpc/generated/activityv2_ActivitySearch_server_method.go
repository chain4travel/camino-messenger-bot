...
func (s *activityv2ActivitySearchServer) ActivitySearch(ctx context.Context, request *activityv2.ActivitySearchRequest) (*activityv2.ActivitySearchResponse, error) {
	...
	responseIntf, err := s.reqHandler.HandleMessageRequest(ctx, ActivitySearchServiceV2Request, request)
	if err != nil {
		return s.errorResponse(err.Error()), nil
	}

	response, ok := responseIntf.(*activityv2.ActivitySearchResponse)
	if !ok {
		return s.errorResponse(fmt.Sprintf("invalid response type: expected %s, got %T", ActivitySearchServiceV2Response, response)), nil
	}

	return response, nil
}

func (s *activityv2ActivitySearchServer) errorResponse(errorMessage string) *activityv2.ActivitySearchResponse {
