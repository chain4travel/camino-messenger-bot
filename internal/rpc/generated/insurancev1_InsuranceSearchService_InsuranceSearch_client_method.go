...
func (s InsuranceSearchServiceV1Client) Call(ctx context.Context, requestIntf protoreflect.ProtoMessage, opts ...grpc.CallOption) (protoreflect.ProtoMessage, types.MessageType, error) {
	messageType := InsuranceSearchServiceV1Response

	request, ok := requestIntf.(*insurancev1.InsuranceSearchRequest)
	if !ok {
		return s.errorResponse(fmt.Sprintf("invalid request type: expected %s, got %T", InsuranceSearchServiceV1Request, requestIntf)), messageType, nil
	}
...
func (s *InsuranceSearchServiceV1Client) ErrorResponseAndType(errorMessage string) (protoreflect.ProtoMessage, types.MessageType) {
	return s.errorResponse(errorMessage), InsuranceSearchServiceV1Response
}

func (s *InsuranceSearchServiceV1Client) errorResponse(errorMessage string) protoreflect.ProtoMessage {
