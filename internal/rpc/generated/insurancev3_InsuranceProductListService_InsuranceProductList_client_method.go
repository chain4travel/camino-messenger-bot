...
func (s InsuranceProductListServiceV3Client) Call(ctx context.Context, requestIntf protoreflect.ProtoMessage, opts ...grpc.CallOption) (protoreflect.ProtoMessage, types.MessageType, error) {
	messageType := InsuranceProductListServiceV3Response

	request, ok := requestIntf.(*insurancev3.InsuranceProductListRequest)
	if !ok {
		return s.errorResponse(fmt.Sprintf("invalid request type: expected %s, got %T", InsuranceProductListServiceV3Request, requestIntf)), messageType, nil
	}
...
func (s *InsuranceProductListServiceV3Client) ErrorResponseAndType(errorMessage string) (protoreflect.ProtoMessage, types.MessageType) {
	return s.errorResponse(errorMessage), InsuranceProductListServiceV3Response
}

func (s *InsuranceProductListServiceV3Client) errorResponse(errorMessage string) protoreflect.ProtoMessage {
