...
func (s *activityv5ActivityProductInfoServer) ActivityProductInfo(ctx context.Context, request *activityv5.ActivityProductInfoRequest) (*activityv5.ActivityProductInfoResponse, error) {
	...
	responseIntf, err := s.reqHandler.HandleMessageRequest(ctx, ActivityProductInfoServiceV5Request, request)
	switch {
	...
	response, ok := responseIntf.(*activityv5.ActivityProductInfoResponse)
	if !ok {
		return s.errorResponse(typesv4.ErrorCode_ERROR_CODE_INTERNAL, fmt.Sprintf("invalid response type: expected %s, got %T", ActivityProductInfoServiceV5Response, responseIntf)), nil
	}

	return response, nil
}

func (s *activityv5ActivityProductInfoServer) errorResponse(code typesv4.ErrorCode, errorMessage string) *activityv5.ActivityProductInfoResponse {
