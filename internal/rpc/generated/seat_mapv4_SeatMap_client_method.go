...
func (s *SeatMapV4Client) Call(ctx context.Context, requestIntf protoreflect.ProtoMessage, opts ...grpc.CallOption) (protoreflect.ProtoMessage, message.Type) {
	messageType := SeatMapServiceV4Response

	request, ok := requestIntf.(*seat_mapv4.SeatMapRequest)
	if !ok {
		return s.errorResponse(typesv4.ErrorCode_ERROR_CODE_INTERNAL, fmt.Sprintf("invalid request type: expected %s, got %T", SeatMapServiceV4Request, requestIntf)), messageType
	}
...
func (s *SeatMapV4Client) InvalidProtoErrResponseAndType(errorMessage string) (protoreflect.ProtoMessage, message.Type) {
	return s.errorResponse(typesv4.ErrorCode_ERROR_CODE_INVALID_PROTO, errorMessage), SeatMapServiceV4Response
}

func (s *SeatMapV4Client) errorResponse(code typesv4.ErrorCode, errorMessage string) *seat_mapv4.SeatMapResponse {
