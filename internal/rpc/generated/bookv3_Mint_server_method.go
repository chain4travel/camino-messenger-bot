...
func (s *bookv3MintServer) Mint(ctx context.Context, request *bookv3.MintRequest) (*bookv3.MintResponse, error) {
	...
	responseIntf, err := s.reqHandler.HandleMessageRequest(ctx, MintV3Request, request)
	if err != nil {
	...
	if !ok {
		return s.errorResponse(fmt.Sprintf("invalid response type: expected %s, got %T", MintV3Response, response)), nil
	}

	return response, nil
}

func (s *bookv3MintServer) errorResponse(errorMessage string) *bookv3.MintResponse {
