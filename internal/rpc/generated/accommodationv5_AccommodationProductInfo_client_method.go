...
func (s *AccommodationProductInfoV5Client) Call(ctx context.Context, requestIntf protoreflect.ProtoMessage, opts ...grpc.CallOption) (protoreflect.ProtoMessage, message.Type) {
	messageType := AccommodationProductInfoServiceV5Response

	request, ok := requestIntf.(*accommodationv5.AccommodationProductInfoRequest)
	if !ok {
		return s.errorResponse(typesv4.ErrorCode_ERROR_CODE_INTERNAL, fmt.Sprintf("invalid request type: expected %s, got %T", AccommodationProductInfoServiceV5Request, requestIntf)), messageType
	}
...
func (s *AccommodationProductInfoV5Client) InvalidProtoErrResponseAndType(errorMessage string) (protoreflect.ProtoMessage, message.Type) {
	return s.errorResponse(typesv4.ErrorCode_ERROR_CODE_INVALID_PROTO, errorMessage), AccommodationProductInfoServiceV5Response
}

func (s *AccommodationProductInfoV5Client) errorResponse(code typesv4.ErrorCode, errorMessage string) *accommodationv5.AccommodationProductInfoResponse {
	return &accommodationv5.AccommodationProductInfoResponse{
		Response: &accommodationv5.AccommodationProductInfoResponse_ErrorResponse{
			ErrorResponse: &accommodationv5.AccommodationProductInfoErrorResponse{
