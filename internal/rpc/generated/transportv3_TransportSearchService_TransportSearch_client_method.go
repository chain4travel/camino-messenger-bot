...
func (s TransportSearchServiceV3Client) Call(ctx context.Context, requestIntf protoreflect.ProtoMessage, opts ...grpc.CallOption) (protoreflect.ProtoMessage, types.MessageType, error) {
	messageType := TransportSearchServiceV3Response

	request, ok := requestIntf.(*transportv3.TransportSearchRequest)
	if !ok {
		return s.errorResponse(fmt.Sprintf("invalid request type: expected %s, got %T", TransportSearchServiceV3Request, requestIntf)), messageType, nil
	}
...
func (s *TransportSearchServiceV3Client) ErrorResponseAndType(errorMessage string) (protoreflect.ProtoMessage, types.MessageType) {
	return s.errorResponse(errorMessage), TransportSearchServiceV3Response
}

func (s *TransportSearchServiceV3Client) errorResponse(errorMessage string) protoreflect.ProtoMessage {
