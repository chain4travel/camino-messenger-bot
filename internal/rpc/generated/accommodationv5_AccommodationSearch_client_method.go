...
func (s *AccommodationSearchV5Client) Call(ctx context.Context, requestIntf protoreflect.ProtoMessage, opts ...grpc.CallOption) (protoreflect.ProtoMessage, message.Type) {
	messageType := AccommodationSearchServiceV5Response

	request, ok := requestIntf.(*accommodationv5.AccommodationSearchRequest)
	if !ok {
		return s.errorResponse(typesv4.ErrorCode_ERROR_CODE_INTERNAL, fmt.Sprintf("invalid request type: expected %s, got %T", AccommodationSearchServiceV5Request, requestIntf)), messageType
	}
...
func (s *AccommodationSearchV5Client) InvalidProtoErrResponseAndType(errorMessage string) (protoreflect.ProtoMessage, message.Type) {
	return s.errorResponse(typesv4.ErrorCode_ERROR_CODE_INVALID_PROTO, errorMessage), AccommodationSearchServiceV5Response
}

func (s *AccommodationSearchV5Client) errorResponse(code typesv4.ErrorCode, errorMessage string) *accommodationv5.AccommodationSearchResponse {
