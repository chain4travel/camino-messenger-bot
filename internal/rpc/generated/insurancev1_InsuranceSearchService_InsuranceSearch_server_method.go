...
func (s *insurancev1InsuranceSearchServiceServer) InsuranceSearch(ctx context.Context, request *insurancev1.InsuranceSearchRequest) (*insurancev1.InsuranceSearchResponse, error) {
	...
	responseIntf, err := s.reqHandler.HandleMessageRequest(ctx, InsuranceSearchServiceV1Request, request)
	if err != nil {
		return s.errorResponse(err.Error()), nil
	}

	response, ok := responseIntf.(*insurancev1.InsuranceSearchResponse)
	if !ok {
		return s.errorResponse(fmt.Sprintf("invalid response type: expected %s, got %T", InsuranceSearchServiceV1Response, response)), nil
	}

	return response, nil
}

func (s *insurancev1InsuranceSearchServiceServer) errorResponse(errorMessage string) *insurancev1.InsuranceSearchResponse {
	return &insurancev1.InsuranceSearchResponse{
