...
func (s AccommodationSearchServiceV4Client) Call(ctx context.Context, requestIntf protoreflect.ProtoMessage, opts ...grpc.CallOption) (protoreflect.ProtoMessage, types.MessageType, error) {
	messageType := AccommodationSearchServiceV4Response

	request, ok := requestIntf.(*accommodationv4.AccommodationSearchRequest)
	if !ok {
		return s.errorResponse(fmt.Sprintf("invalid request type: expected %s, got %T", AccommodationSearchServiceV4Request, requestIntf)), messageType, nil
	}
...
func (s *AccommodationSearchServiceV4Client) ErrorResponseAndType(errorMessage string) (protoreflect.ProtoMessage, types.MessageType) {
	return s.errorResponse(errorMessage), AccommodationSearchServiceV4Response
}

func (s *AccommodationSearchServiceV4Client) errorResponse(errorMessage string) protoreflect.ProtoMessage {
