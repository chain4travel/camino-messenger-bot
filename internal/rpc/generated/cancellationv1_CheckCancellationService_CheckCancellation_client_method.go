...
func (s CheckCancellationServiceV1Client) Call(ctx context.Context, requestIntf protoreflect.ProtoMessage, opts ...grpc.CallOption) (protoreflect.ProtoMessage, types.MessageType, error) {
	messageType := CheckCancellationServiceV1Response

	request, ok := requestIntf.(*cancellationv1.CheckCancellationRequest)
	if !ok {
		return s.errorResponse(fmt.Sprintf("invalid request type: expected %s, got %T", CheckCancellationServiceV1Request, requestIntf)), messageType, nil
	}
...
func (s *CheckCancellationServiceV1Client) ErrorResponseAndType(errorMessage string) (protoreflect.ProtoMessage, types.MessageType) {
	return s.errorResponse(errorMessage), CheckCancellationServiceV1Response
}

func (s *CheckCancellationServiceV1Client) errorResponse(errorMessage string) protoreflect.ProtoMessage {
