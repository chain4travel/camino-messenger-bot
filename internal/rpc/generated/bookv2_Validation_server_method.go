...
func (s *bookv2ValidationServer) Validation(ctx context.Context, request *bookv2.ValidationRequest) (*bookv2.ValidationResponse, error) {
	...
	responseIntf, err := s.reqHandler.HandleMessageRequest(ctx, ValidationV2Request, request)
	if err != nil {
	...
	if !ok {
		return s.errorResponse(fmt.Sprintf("invalid response type: expected %s, got %T", ValidationV2Response, response)), nil
	}

	return response, nil
}

func (s *bookv2ValidationServer) errorResponse(errorMessage string) *bookv2.ValidationResponse {
