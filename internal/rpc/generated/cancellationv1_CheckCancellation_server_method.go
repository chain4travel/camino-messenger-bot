...
func (s *cancellationv1CheckCancellationServer) CheckCancellation(ctx context.Context, request *cancellationv1.CheckCancellationRequest) (*cancellationv1.CheckCancellationResponse, error) {
	...
	responseIntf, err := s.reqHandler.HandleMessageRequest(ctx, CheckCancellationV1Request, request)
	if err != nil {
	...
	if !ok {
		return s.errorResponse(fmt.Sprintf("invalid response type: expected %s, got %T", CheckCancellationV1Response, response)), nil
	}

	return response, nil
}

func (s *cancellationv1CheckCancellationServer) errorResponse(errorMessage string) *cancellationv1.CheckCancellationResponse {
