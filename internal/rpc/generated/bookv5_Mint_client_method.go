...
func (s *MintV5Client) Call(ctx context.Context, requestIntf protoreflect.ProtoMessage, opts ...grpc.CallOption) (protoreflect.ProtoMessage, message.Type) {
	messageType := MintServiceV5Response

	request, ok := requestIntf.(*bookv5.MintRequest)
	if !ok {
		return s.errorResponse(typesv4.ErrorCode_ERROR_CODE_INTERNAL, fmt.Sprintf("invalid request type: expected %s, got %T", MintServiceV5Request, requestIntf)), messageType
	}
...
func (s *MintV5Client) InvalidProtoErrResponseAndType(errorMessage string) (protoreflect.ProtoMessage, message.Type) {
	return s.errorResponse(typesv4.ErrorCode_ERROR_CODE_INVALID_PROTO, errorMessage), MintServiceV5Response
}

func (s *MintV5Client) errorResponse(code typesv4.ErrorCode, errorMessage string) *bookv5.MintResponse {
