...
func (s InsuranceSearchServiceV3Client) Call(ctx context.Context, requestIntf protoreflect.ProtoMessage, opts ...grpc.CallOption) (protoreflect.ProtoMessage, types.MessageType, error) {
	messageType := InsuranceSearchServiceV3Response

	request, ok := requestIntf.(*insurancev3.InsuranceSearchRequest)
	if !ok {
		return s.errorResponse(fmt.Sprintf("invalid request type: expected %s, got %T", InsuranceSearchServiceV3Request, requestIntf)), messageType, nil
	}
...
func (s *InsuranceSearchServiceV3Client) ErrorResponseAndType(errorMessage string) (protoreflect.ProtoMessage, types.MessageType) {
	return s.errorResponse(errorMessage), InsuranceSearchServiceV3Response
}

func (s *InsuranceSearchServiceV3Client) errorResponse(errorMessage string) protoreflect.ProtoMessage {
