...
func (s *insurancev1InsuranceProductInfoServiceServer) InsuranceProductInfo(ctx context.Context, request *insurancev1.InsuranceProductInfoRequest) (*insurancev1.InsuranceProductInfoResponse, error) {
	...
	responseIntf, err := s.reqHandler.HandleMessageRequest(ctx, InsuranceProductInfoServiceV1Request, request)
	if err != nil {
		return s.errorResponse(err.Error()), nil
	}

	response, ok := responseIntf.(*insurancev1.InsuranceProductInfoResponse)
	if !ok {
		return s.errorResponse(fmt.Sprintf("invalid response type: expected %s, got %T", InsuranceProductInfoServiceV1Response, response)), nil
	}

	return response, nil
}

func (s *insurancev1InsuranceProductInfoServiceServer) errorResponse(errorMessage string) *insurancev1.InsuranceProductInfoResponse {
