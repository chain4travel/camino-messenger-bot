...
func (s *bookv5MintServer) Mint(ctx context.Context, request *bookv5.MintRequest) (*bookv5.MintResponse, error) {
	...
	responseIntf, err := s.reqHandler.HandleMessageRequest(ctx, MintServiceV5Request, request)
	switch {
	...
	response, ok := responseIntf.(*bookv5.MintResponse)
	if !ok {
		return s.errorResponse(typesv4.ErrorCode_ERROR_CODE_INTERNAL, fmt.Sprintf("invalid response type: expected %s, got %T", MintServiceV5Response, responseIntf)), nil
	}

	return response, nil
}

func (s *bookv5MintServer) errorResponse(code typesv4.ErrorCode, errorMessage string) *bookv5.MintResponse {
