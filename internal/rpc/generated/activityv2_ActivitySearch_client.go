// Code generated by 'scripts/generate_grpc_service_handlers.sh'. DO NOT EDIT.
// template: templates/v1/client.go.tpl

package generated

import (
	"buf.build/gen/go/chain4travel/camino-messenger-protocol/grpc/go/cmp/services/activity/v2/activityv2grpc"
	"github.com/chain4travel/camino-messenger-bot/v12/internal/messaging/message"
	"github.com/chain4travel/camino-messenger-bot/v12/internal/rpc"

	"google.golang.org/grpc"
)

const (
	ActivitySearchServiceV2                      = "cmp.services.activity.v2.ActivitySearchService"
	ActivitySearchServiceV2Request  message.Type = message.Type(ActivitySearchServiceV2 + ".Request")
	ActivitySearchServiceV2Response message.Type = message.Type(ActivitySearchServiceV2 + ".Response")
)

var _ rpc.Client = (*ActivitySearchV2Client)(nil)

func NewActivitySearchServiceV2Client(grpcCon *grpc.ClientConn) *ActivitySearchV2Client {
	client := activityv2grpc.NewActivitySearchServiceClient(grpcCon)
	return &ActivitySearchV2Client{client: client}
}

type ActivitySearchV2Client struct {
	client activityv2grpc.ActivitySearchServiceClient
}
