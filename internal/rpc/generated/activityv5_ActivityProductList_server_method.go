...
func (s *activityv5ActivityProductListServer) ActivityProductList(ctx context.Context, request *activityv5.ActivityProductListRequest) (*activityv5.ActivityProductListResponse, error) {
	...
	responseIntf, err := s.reqHandler.HandleMessageRequest(ctx, ActivityProductListServiceV5Request, request)
	switch {
	...
	response, ok := responseIntf.(*activityv5.ActivityProductListResponse)
	if !ok {
		return s.errorResponse(typesv4.ErrorCode_ERROR_CODE_INTERNAL, fmt.Sprintf("invalid response type: expected %s, got %T", ActivityProductListServiceV5Response, responseIntf)), nil
	}

	return response, nil
}

func (s *activityv5ActivityProductListServer) errorResponse(code typesv4.ErrorCode, errorMessage string) *activityv5.ActivityProductListResponse {
