...
func (s ValidationServiceV4Client) Call(ctx context.Context, requestIntf protoreflect.ProtoMessage, opts ...grpc.CallOption) (protoreflect.ProtoMessage, types.MessageType, error) {
	messageType := ValidationServiceV4Response

	request, ok := requestIntf.(*bookv4.ValidationRequest)
	if !ok {
		return s.errorResponse(fmt.Sprintf("invalid request type: expected %s, got %T", ValidationServiceV4Request, requestIntf)), messageType, nil
	}
...
func (s *ValidationServiceV4Client) ErrorResponseAndType(errorMessage string) (protoreflect.ProtoMessage, types.MessageType) {
	return s.errorResponse(errorMessage), ValidationServiceV4Response
}

func (s *ValidationServiceV4Client) errorResponse(errorMessage string) protoreflect.ProtoMessage {
