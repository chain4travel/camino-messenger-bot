...
func (s *seat_mapv4SeatMapServer) SeatMap(ctx context.Context, request *seat_mapv4.SeatMapRequest) (*seat_mapv4.SeatMapResponse, error) {
	...
	responseIntf, err := s.reqHandler.HandleMessageRequest(ctx, SeatMapV4Request, request)
	if err != nil {
	...
	if !ok {
		return s.errorResponse(fmt.Sprintf("invalid response type: expected %s, got %T", SeatMapV4Response, response)), nil
	}

	return response, nil
}

func (s *seat_mapv4SeatMapServer) errorResponse(errorMessage string) *seat_mapv4.SeatMapResponse {
