...
func (s *activityv5ActivitySearchServer) ActivitySearch(ctx context.Context, request *activityv5.ActivitySearchRequest) (*activityv5.ActivitySearchResponse, error) {
	...
	responseIntf, err := s.reqHandler.HandleMessageRequest(ctx, ActivitySearchServiceV5Request, request)
	switch {
	...
	response, ok := responseIntf.(*activityv5.ActivitySearchResponse)
	if !ok {
		return s.errorResponse(typesv4.ErrorCode_ERROR_CODE_INTERNAL, fmt.Sprintf("invalid response type: expected %s, got %T", ActivitySearchServiceV5Response, responseIntf)), nil
	}

	return response, nil
}

func (s *activityv5ActivitySearchServer) errorResponse(code typesv4.ErrorCode, errorMessage string) *activityv5.ActivitySearchResponse {
