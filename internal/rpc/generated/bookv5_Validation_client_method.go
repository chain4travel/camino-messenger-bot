...
func (s *ValidationV5Client) Call(ctx context.Context, requestIntf protoreflect.ProtoMessage, opts ...grpc.CallOption) (protoreflect.ProtoMessage, message.Type) {
	messageType := ValidationServiceV5Response

	request, ok := requestIntf.(*bookv5.ValidationRequest)
	if !ok {
		return s.errorResponse(typesv4.ErrorCode_ERROR_CODE_INTERNAL, fmt.Sprintf("invalid request type: expected %s, got %T", ValidationServiceV5Request, requestIntf)), messageType
	}
...
func (s *ValidationV5Client) InvalidProtoErrResponseAndType(errorMessage string) (protoreflect.ProtoMessage, message.Type) {
	return s.errorResponse(typesv4.ErrorCode_ERROR_CODE_INVALID_PROTO, errorMessage), ValidationServiceV5Response
}

func (s *ValidationV5Client) errorResponse(code typesv4.ErrorCode, errorMessage string) *bookv5.ValidationResponse {
