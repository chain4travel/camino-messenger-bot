...
func (s *bookv5ValidationServer) Validation(ctx context.Context, request *bookv5.ValidationRequest) (*bookv5.ValidationResponse, error) {
	...
	responseIntf, err := s.reqHandler.HandleMessageRequest(ctx, ValidationServiceV5Request, request)
	switch {
	...
	response, ok := responseIntf.(*bookv5.ValidationResponse)
	if !ok {
		return s.errorResponse(typesv4.ErrorCode_ERROR_CODE_INTERNAL, fmt.Sprintf("invalid response type: expected %s, got %T", ValidationServiceV5Response, responseIntf)), nil
	}

	return response, nil
}

func (s *bookv5ValidationServer) errorResponse(code typesv4.ErrorCode, errorMessage string) *bookv5.ValidationResponse {
