...
func (s *activityv5ActivityProductShortListServer) ActivityProductShortList(ctx context.Context, request *activityv5.ActivityProductShortListRequest) (*activityv5.ActivityProductShortListResponse, error) {
	...
	responseIntf, err := s.reqHandler.HandleMessageRequest(ctx, ActivityProductShortListServiceV5Request, request)
	switch {
	...
	response, ok := responseIntf.(*activityv5.ActivityProductShortListResponse)
	if !ok {
		return s.errorResponse(typesv4.ErrorCode_ERROR_CODE_INTERNAL, fmt.Sprintf("invalid response type: expected %s, got %T", ActivityProductShortListServiceV5Response, responseIntf)), nil
	}

	return response, nil
}

func (s *activityv5ActivityProductShortListServer) errorResponse(code typesv4.ErrorCode, errorMessage string) *activityv5.ActivityProductShortListResponse {
