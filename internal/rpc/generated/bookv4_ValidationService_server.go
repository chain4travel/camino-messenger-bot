// Code generated by 'scripts/generate_grpc_service_handlers.sh'. DO NOT EDIT.
// template: templates/server.go.tpl

package generated

import (
	"buf.build/gen/go/chain4travel/camino-messenger-protocol/grpc/go/cmp/services/book/v4/bookv4grpc"
	"github.com/chain4travel/camino-messenger-bot/v12/internal/rpc"

	"google.golang.org/grpc"
)

var _ bookv4grpc.ValidationServiceServer = (*bookv4ValidationServiceServer)(nil)

type bookv4ValidationServiceServer struct {
	reqHandler rpc.RequestHandler
}

func registerValidationServiceV4Server(grpcServer *grpc.Server, reqHandler rpc.RequestHandler) {
	bookv4grpc.RegisterValidationServiceServer(grpcServer, &bookv4ValidationServiceServer{reqHandler})
}
