...
func (s *activityv2ActivityProductInfoServer) ActivityProductInfo(ctx context.Context, request *activityv2.ActivityProductInfoRequest) (*activityv2.ActivityProductInfoResponse, error) {
	...
	responseIntf, err := s.reqHandler.HandleMessageRequest(ctx, ActivityProductInfoV2Request, request)
	if err != nil {
	...
	if !ok {
		return s.errorResponse(fmt.Sprintf("invalid response type: expected %s, got %T", ActivityProductInfoV2Response, response)), nil
	}
