// Code generated by 'scripts/generate_grpc_service_handlers.sh'. DO NOT EDIT.
// template: templates/v1/client.go.tpl

package generated

import (
	"buf.build/gen/go/chain4travel/camino-messenger-protocol/grpc/go/cmp/services/accommodation/v2/accommodationv2grpc"
	"github.com/chain4travel/camino-messenger-bot/v12/internal/messaging/message"
	"github.com/chain4travel/camino-messenger-bot/v12/internal/rpc"

	"google.golang.org/grpc"
)

const (
	AccommodationProductListServiceV2                      = "cmp.services.accommodation.v2.AccommodationProductListService"
	AccommodationProductListServiceV2Request  message.Type = message.Type(AccommodationProductListServiceV2 + ".Request")
	AccommodationProductListServiceV2Response message.Type = message.Type(AccommodationProductListServiceV2 + ".Response")
)

var _ rpc.Client = (*AccommodationProductListV2Client)(nil)

func NewAccommodationProductListServiceV2Client(grpcCon *grpc.ClientConn) *AccommodationProductListV2Client {
	client := accommodationv2grpc.NewAccommodationProductListServiceClient(grpcCon)
	return &AccommodationProductListV2Client{client: client}
}

type AccommodationProductListV2Client struct {
	client accommodationv2grpc.AccommodationProductListServiceClient
}
