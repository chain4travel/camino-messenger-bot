...
func (s *activityv4ActivitySearchServer) ActivitySearch(ctx context.Context, request *activityv4.ActivitySearchRequest) (*activityv4.ActivitySearchResponse, error) {
	...
	responseIntf, err := s.reqHandler.HandleMessageRequest(ctx, ActivitySearchV4Request, request)
	if err != nil {
	...
	if !ok {
		return s.errorResponse(fmt.Sprintf("invalid response type: expected %s, got %T", ActivitySearchV4Response, response)), nil
	}
