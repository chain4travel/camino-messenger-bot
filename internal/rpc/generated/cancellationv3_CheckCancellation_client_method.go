...
func (s *CheckCancellationV3Client) Call(ctx context.Context, requestIntf protoreflect.ProtoMessage, opts ...grpc.CallOption) (protoreflect.ProtoMessage, message.Type) {
	messageType := CheckCancellationServiceV3Response

	request, ok := requestIntf.(*cancellationv3.CheckCancellationRequest)
	if !ok {
		return s.errorResponse(typesv4.ErrorCode_ERROR_CODE_INTERNAL, fmt.Sprintf("invalid request type: expected %s, got %T", CheckCancellationServiceV3Request, requestIntf)), messageType
	}
...
func (s *CheckCancellationV3Client) InvalidProtoErrResponseAndType(errorMessage string) (protoreflect.ProtoMessage, message.Type) {
	return s.errorResponse(typesv4.ErrorCode_ERROR_CODE_INVALID_PROTO, errorMessage), CheckCancellationServiceV3Response
}

func (s *CheckCancellationV3Client) errorResponse(code typesv4.ErrorCode, errorMessage string) *cancellationv3.CheckCancellationResponse {
