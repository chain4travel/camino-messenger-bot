...
func (s *seat_mapv2SeatMapServer) SeatMap(ctx context.Context, request *seat_mapv2.SeatMapRequest) (*seat_mapv2.SeatMapResponse, error) {
	...
	responseIntf, err := s.reqHandler.HandleMessageRequest(ctx, SeatMapV2Request, request)
	if err != nil {
	...
	if !ok {
		return s.errorResponse(fmt.Sprintf("invalid response type: expected %s, got %T", SeatMapV2Response, response)), nil
	}

	return response, nil
}

func (s *seat_mapv2SeatMapServer) errorResponse(errorMessage string) *seat_mapv2.SeatMapResponse {
