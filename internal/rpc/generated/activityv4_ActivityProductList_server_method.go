...
func (s *activityv4ActivityProductListServer) ActivityProductList(ctx context.Context, request *activityv4.ActivityProductListRequest) (*activityv4.ActivityProductListResponse, error) {
	...
	if !ok {
		return s.errorResponse(typesv4.ErrorCode_ERROR_CODE_INTERNAL, fmt.Sprintf("invalid response type: expected %s, got %T", ActivityProductListServiceV4Response, responseIntf)), nil
	}

	return response, nil
}

func (s *activityv4ActivityProductListServer) errorResponse(code typesv4.ErrorCode, errorMessage string) *activityv4.ActivityProductListResponse {
