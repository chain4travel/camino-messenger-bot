...
func (h *evmResponseHandler) prepareMintResponseV3(
	...
	if !common.IsHexAddress(request.BuyerAddress.Address) {
		...
		h.responseHeaderHandler.AddErrorV1(response.Header, errMsg)
		return
	}
	...
	if err != nil {
		h.logger.Error(err)
		h.responseHeaderHandler.AddErrorV1(response.Header, err.Error())
		return
	}
	...
	if err != nil {
		...
		h.responseHeaderHandler.AddErrorV1(response.Header, errMessage)
		return
	}
...
func (h *evmResponseHandler) processMintResponseV3(ctx context.Context, response *bookv3.MintResponse) {
	if response.MintTransactionId == nil || response.MintTransactionId.Hash == "" {
		...
		h.responseHeaderHandler.AddErrorV1(response.Header, errMissingMintTxID.Error())
		return
	}
