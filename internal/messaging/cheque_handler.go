...
type ChequeHandler interface {
	...
	IsBotAllowed(fromBot common.Address) (bool, error)
...
func (cm *evmChequeHandler) IssueCheque(ctx context.Context, fromCMAccount common.Address, toCMAccount common.Address, fromBot common.Address, toBot common.Address, amount uint64) (cheques.SignedCheque, error) {
	...
	if err != nil {
		return cheques.SignedCheque{}, fmt.Errorf("failed to get previous cheque: %v", err)
	}
	...
	if err != nil {
		cm.logger.Errorf("Failed to call GetLastCashIn: %v", err)
...
func (cm *evmChequeHandler) newStorageSession(ctx context.Context) (storage.Session, error) {
	...
	// Ensure the session is committed at the end if no errors occur
	defer func() {
		if err == nil {
			if commitErr := session.Commit(); commitErr != nil {
				cm.logger.Errorf("Failed to commit session: %v", commitErr)
				// Handle commit error if necessary
			}
		}
	}()
...
func (cm *evmChequeHandler) IsBotAllowed(fromBot common.Address) (bool, error) {
	callOpts := &bind.CallOpts{
