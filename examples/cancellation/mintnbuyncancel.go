...
import (
	...
	"github.com/chain4travel/camino-messenger-bot/pkg/cancellation"
...
func main() {
	...
	distributorAddress := common.HexToAddress("0x09E5F8D48AA47883eAf044F7C062D32fA5A92C2f")
	receipt, err := bs.MintBookingToken(
		context.Background(),
		distributorAddress, // reservedFor address
		tokenURI,
		expiration,
		priceBigInt,
		paymentToken,
	...
	pkStringDistributor := flag.String("pkDistributor", "7dcea645faf893478f84cd76096e26a65c79dfa3c4ac4aa1548361909f539eae", "Private Key without the 0x notation")
	pk, err = crypto.HexToECDSA(*pkStringDistributor)

	bs, err = booking.NewService(distributorAddress, pk, client, sugar, erc20, cmAccounts)
	if err != nil {
		sugar.Fatalf("Failed to create Booking Service: %v", err)
	}
	...
	// Cancel a new booking token

	cs, err := cancellation.NewService(cmAccountAddr, pk, client, sugar, erc20, cmAccounts)

	cancelReceipt, err := cs.InitiateCancellationProposal(
		context.Background(),
		tokenID,
		priceBigInt,
	)
	...
	// Parse the logs
	for _, bLog := range cancelReceipt.Logs {
		event, err := bt.ParseTokenBought(*bLog)
		if err == nil {
