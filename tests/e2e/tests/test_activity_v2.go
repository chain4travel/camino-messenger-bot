...
func (tt *TestActivityV2) Run(t *testing.T) {
	...
	t.Run("Search->Validate->Mint->VerifyBlockchain", func(t *testing.T) {
		...
		verifyBookingTokenStateWithPriceV2(ctx, t, tt.Environment, tt.distributorBot, tokenID, mintRespPrice, balanceBefore)
	})
}
...
func testActivityV2SearchServiceWithTravelPeriod(
	...
	return resp.Metadata.SearchId.Value, resultID, protoPriceBigV2(t, resp.Results[0].Price)
}
