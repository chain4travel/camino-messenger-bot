...
// func testMintV2FullWorkflow(ctx context.Context, t *testing.T, tt *Environment, ppEventStream events.EventsService_SubscribeClient, distributorBot *bot.Bot, supplierBot *bot.Bot) {
...
// func testMintV2TokenExpiredCase(ctx context.Context, t *testing.T, tt *Environment, ppEventStream events.EventsService_SubscribeClient, distributorBot *bot.Bot, supplierBot *bot.Bot) {
// 	// Don't mind the eventStream receives without further processing.
// 	// We just receive all the messages from the pp-mock event stream without any
// 	// further checks as we're only really interested in the last one.
...
// func TestMintV2(t *testing.T, tt *Environment) {
