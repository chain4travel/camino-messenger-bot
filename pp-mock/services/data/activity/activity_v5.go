...
func VerifyV5(extended []*activityv5.ActivityExtendedInfo, searchResults []*activityv5.ActivitySearchResult) {
	expectedCount := 0
		...
		for _, unit := range activity.Units {
			for _, service := range activity.Services {
				...
				for _, searchResultZoneCode := range searchResult.ZoneCodes {
					if !slices.ContainsFunc(activity.Zones, func(z *activityv5.TransferZone) bool {
