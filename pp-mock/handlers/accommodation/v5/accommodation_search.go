...
var _ accommodationv5grpc.AccommodationSearchServiceServer = (*accommodationSearchV5Server)(nil)

type accommodationSearchV5Server struct{}

func NewAccommodationSearchServer() accommodationv5grpc.AccommodationSearchServiceServer {
	return &accommodationSearchV5Server{}
}
...
func (s *accommodationSearchV5Server) AccommodationSearch(_ context.Context, req *accommodationv5.AccommodationSearchRequest) (*accommodationv5.AccommodationSearchResponse, error) {
	...
	searchResults := []*accommodationv5.AccommodationSearchResult{}
	resultIDnum := uint32(0)
	validationPrices := []*state.UnifiedPrice{}

	// loop request queries
	filteredProps := filterExtendedPropertiesByGeoTreeLocation(mockdata.PropertiesV5, req.SearchParametersAccommodation.GetLocationGeoTree())
	...
	for _, prop := range filteredProps {
		for _, room := range prop.Rooms {
			...
			unit := &accommodationv5.Unit{
				...
				Beds:         room.Beds,
				PriceDetail: &typesv5.PriceDetail{
					Price: &typesv5.Price{
					...
					ChargeType: typesv5.ChargeType_CHARGE_TYPE_PER_UNIT,
				},
				Services: []*typesv5.ServiceFact{{ // temporary placeholder
					Code: "MOCK",
					PriceDetail: &typesv5.PriceDetail{
						Price: &typesv5.Price{
							Value:    "100",
						...
						ChargeType:  typesv5.ChargeType_CHARGE_TYPE_PER_PERSON,
			...
			searchResults = append(searchResults, &accommodationv5.AccommodationSearchResult{
				ResultId: resultIDnum,
				TotalPrice: &typesv5.TotalPrice{
					Value: unit.PriceDetail.Price,
					CancelPolicy: &typesv5.CancelPolicy{
						PolicyType: &typesv5.CancelPolicy_ComplexCancelPenalties{
							ComplexCancelPenalties: &typesv5.ComplexCancelPenalties{
	...
	resp := &accommodationv5.AccommodationSearchResponse{
		Response: &accommodationv5.AccommodationSearchResponse_SuccessResponse{
			SuccessResponse: &accommodationv5.AccommodationSearchSuccessResponse{
